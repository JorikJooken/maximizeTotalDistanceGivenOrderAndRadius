...
int main(int argc, char ** argv) {
    ...
    // last step: how to combine the two centers

    long long bestVal=-inf;

    int correspondingN1=-1;
    int correspondingN2=-1;
    for(int n1=1; 2*n1<=n-2; n1++)
    {
        int n2=n-2-n1;
       
        long long val=(n1+1)*(n2+1)+bestProfit[nbItems][n1]+bestProfit[nbItems][n2];
        if(val>bestVal)
        {
            bestVal=val;
            correspondingN1=n1;
    ...
    cout << "Start output for case where the tree has two centres:" << endl;
    cout << "Maximum Wiener index of a graph with " << n << " vertices and radius " << r+1 << " is: " << bestVal << endl;
